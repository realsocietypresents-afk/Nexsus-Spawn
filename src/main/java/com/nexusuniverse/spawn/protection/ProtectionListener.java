...
public class ProtectionListener implements Listener {
    ...
    /**
     * PATCH (v1.0.1): this used to run at the default priority with ignoreCancelled = true, which
     * meant that if anything else on the server -- NexusRealms' own land-claim protection being
     * the obvious one, since the original request explicitly noted this zone needs to apply "even
     * though it's already offered in the server" -- denied the interaction first, this handler
     * never even ran and couldn't override that denial. Chests (and every other allow-listed
     * amenity) were silently at the mercy of whichever other plugin's rules happened to apply to
     * the same coordinates, which is the opposite of what a FORCEFUL zone is supposed to do.
     *
     * Now runs at HIGHEST priority (after every other plugin's normal-priority checks have already
     * made their decision) and does NOT ignore an already-cancelled event, so NexusSpawn's own
     * allow/deny call is always the one that actually sticks inside its zone -- explicitly forcing
     * ALLOW for the six amenities, and explicitly forcing DENY for everything else, regardless of
     * what any other protection system already decided.
     */
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (isAllowedInteraction(block)) {
            // force-allow, overriding a DENY any other plugin may have already set for this block
